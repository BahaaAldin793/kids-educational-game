...
public class _CardGameManager : MonoBehaviour
{
    ...
    [SerializeField]
    private WinStarsUI winStarsManager;
    [SerializeField]
    ...
    // NEW: حدود الوقت لتحديد عدد النجوم - (يمكنك تعديل هذه القيم لاحقاً)
    private const float TimeLimit3Stars = 15f;
    private const float TimeLimit2Stars = 30f;
    ...
    private void CheckGameWin()
    {
        ...
        if (cardLeft == 0)
        {
            ...
            if (time > TimeLimit2Stars)
            {
                starsToAward = 1; // وقت بطيء (30 ثانية فأكثر)
            }
            else if (time > TimeLimit3Stars)
            {
                starsToAward = 2; // وقت متوسط (من 15 إلى 30 ثانية)
            }
            // إذا كان الوقت أقل من 15 ثانية، تبقى starsToAward = 3

            // استدعاء دالة عرض النجوم مع تمرير عدد النجوم
            if (winStarsManager != null)
            {
                winStarsManager.ShowStars(starsToAward);
            }
